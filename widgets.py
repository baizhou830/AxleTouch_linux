...
from tools import _log_to_json, get_base_path, capture_screen, is_qt_wayland, is_wayland_session
...
_WAYLAND_SESSION = is_wayland_session()


def _apply_rounded_mask(widget, radius):
    if is_qt_wayland():
        return
    path = QPainterPath()
...
def _play_wav_linux(wav_path):
    for player in (["paplay"], ["aplay", "-q"], ["ffplay", "-nodisp", "-autoexit"]):
        try:
            ...
            continue
    print("TTS 播放失败：未找到可用的音频播放器（paplay/aplay/ffplay）")
    return None
...
class InteractionDialog(QDialog):
    interaction_selected = pyqtSignal(dict)

    def __init__(self, parent=None):
        super().__init__(parent)
        self._drag_pos = None
        self.setWindowTitle("和雨竹互动")
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.Tool)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setFixedSize(440, 460)
        self._interactions = load_interactions()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        hint = QLabel("选择一个互动方式，雨竹会做出回应~")
        hint.setStyleSheet("color: #787d88; font-size: 11px;")
        hint.setContentsMargins(20, 4, 20, 8)
        ...
        self._name_input.setPlaceholderText("互动名称")
        self._name_input.setStyleSheet(INPUT_STYLE_FEED)
        add_row.addWidget(self._name_input, stretch=1)

        self._prompt_input = QLineEdit()
        self._prompt_input.setPlaceholderText("发给雨竹的Prompt")
        self._prompt_input.setStyleSheet(INPUT_STYLE_FEED)
        add_row.addWidget(self._prompt_input, stretch=2)
        ...
        add_row.addWidget(add_btn)
        box_layout.addLayout(add_row)

        self._list = QListWidget()
        self._list.setStyleSheet(LIST_STYLE_FEED)
        box_layout.addWidget(self._list)

        btn_row = QHBoxLayout()
        btn_row.setSpacing(8)

        act_btn = QPushButton("互动")
        act_btn.setStyleSheet(ACCENT_BTN_STYLE_FEED)
        act_btn.setCursor(QCursor(Qt.PointingHandCursor))
        act_btn.clicked.connect(self._pick_selected)
        btn_row.addWidget(act_btn)

        del_btn = QPushButton("删除")
        del_btn.setStyleSheet(DANGER_BTN_STYLE_FEED)
        del_btn.setCursor(QCursor(Qt.PointingHandCursor))
        del_btn.clicked.connect(self._remove_interaction)
        btn_row.addWidget(del_btn)
    ...
    def _add_interaction(self):
        ...
        if not name:
            QMessageBox.warning(self, "提示", "请输入互动名称")
            return
        if not prompt:
            QMessageBox.warning(self, "提示", "请输入互动提示")
            return
        if any(i["name"] == name for i in self._interactions):
            QMessageBox.warning(self, "提示", f"已存在名为「{name}」的互动")
            return
        self._interactions.append({"name": name, "prompt": prompt})
        self._persist()
    ...
    def _remove_interaction(self):
        ...
        if not item:
            QMessageBox.warning(self, "提示", "请先选择一个互动")
            return
        it = item.data(Qt.UserRole)
        reply = QMessageBox.question(
            self, "确认删除", f"确定要删除互动「{it['name']}」吗？",
            QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
            self._interactions = [i for i in self._interactions if i["name"] != it["name"]]
            self._persist()
            self._refresh_list()
    ...
    def _pick_selected(self):
        ...
        if not item:
            ...
            return
        it = item.data(Qt.UserRole)
        self.interaction_selected.emit(it)
        self.accept()

    def _persist(self):
        try:
            save_interactions(self._interactions)
        except Exception:
            pass
...
class EdgeFloatingBlock(QWidget):
    ...
    def _on_tts_audio_ready(self, audio_data):
        ...
        temp_path = os.path.join(
            tempfile.gettempdir(), f"axletouch_tts_{os.getpid()}.wav")
        try:
            with open(temp_path, "wb") as f:
                f.write(audio_data)
            self._player_proc = _play_wav_linux(temp_path)
    ...
    def contextMenuEvent(self, event):
        ...
        act_action = QAction("互动",self)
        act_action.triggered.connect(self._on_interact)
    ...
    def capture(self):
        try:
            ...
            print("截图失败：无法获取屏幕图像")
            self._content_bar.show_content("截图失败了，看看日志吧~")
            return

        if pixmap.isNull():
            print("截图失败：获取的图像无效")
            hint = ("Wayland 下需要安装 grim / gnome-screenshot / spectacle 之一"
                    "才能截图~") if _WAYLAND_SESSION else "截图失败：获取的图像无效"
            self._content_bar.show_content(hint)
            return
    ...
    def _on_interact(self):
        ...
        dialog.interaction_selected.connect(self._on_interaction_selected)
        dialog.exec_()

    def _on_interaction_selected(self, interaction):
        name = interaction.get("name", "")
        prompt = interaction.get("prompt", "")
        print(" -----[ Action ]----- ", "\n",
              "[", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "]", "\n",
              "\"", f"你对雨竹进行了互动：{name}", "\"","\n")
        if self._ai and prompt:
            self._ai.send_message(prompt)
